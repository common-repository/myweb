(function($,window,document,undefined){'use strict';var Myweb_Accessibility_App={cache:{$document:$(document),$window:$(window)},cacheElements:function(){this.cache.$toolbar=$('#myweb-ac-toolbar');this.cache.$toolbarLinks=this.cache.$toolbar.find('a.myweb-ac-toolbar-link');this.cache.$toolbarToolsLinks=this.cache.$toolbar.find('.myweb-ac-tools a.myweb-ac-toolbar-link');this.cache.$btnToolbarToggle=this.cache.$toolbar.find('div.myweb-ac-toolbar-toggle > a');this.cache.$skipToContent=$('#myweb-ac-skip-content');this.cache.$body=$('body')},settings:{minFontSize:120,maxFontSize:200,buttonsClassPrefix:'myweb-ac-btn-',bodyClassPrefix:'myweb-ac-',bodyFontClassPrefix:'myweb-ac-resize-font-',storageKey:'myweb-ac',expires:MywebACOptions.save_expiration?MywebACOptions.save_expiration*36e5:43200000},variables:{currentFontSize:120,currentSchema:null},activeActions:{},buildElements:function(){this.cache.$body.prepend(this.cache.$toolbar);this.cache.$body.prepend(this.cache.$skipToContent)},bindEvents:function(){var $self=this;$self.cache.$btnToolbarToggle.on('click',function(event){event.preventDefault();$self.cache.$toolbar.toggleClass('myweb-ac-toolbar-open');if($self.cache.$toolbar.hasClass('myweb-ac-toolbar-open')){$self.cache.$toolbarLinks.attr('tabindex','0')}else{$self.cache.$toolbarLinks.attr('tabindex','-1')}});$(document).on('keyup',function(event){var TAB_KEY=9;if(TAB_KEY!==event.which||!$self.cache.$btnToolbarToggle.is(':focus')){return}
$self.cache.$toolbar.addClass('myweb-ac-toolbar-open');$self.cache.$toolbarLinks.attr('tabindex','0')});$self.bindToolbarButtons()},bindToolbarButtons:function(){var self=this;self.cache.$toolbarToolsLinks.on('click',function(event){event.preventDefault();var $this=$(this),action=$this.data('action'),actionGroup=$this.data('action-group'),deactivate=!1;if('reset'===action){self.reset();return}
...
localStorage.removeItem(this.settings.storageKey)},show_overlay:function(){document.getElementById('myweb-overlay-window').style.display='block'},saveToLocalStorage:function(){if('1'!==MywebACOptions.enable_save){return}
if(!this.variables.expires){this.variables.expires=(new Date()).getTime()+this.settings.expires}
var data={actions:this.activeActions,variables:{currentFontSize:this.variables.currentFontSize,expires:this.variables.expires}};localStorage.setItem(this.settings.storageKey,JSON.stringify(data))},setFromLocalStorage:function(){if('1'!==MywebACOptions.enable_save){return}
...
$.extend(this.variables,localData.variables);for(var action in actions){if(actions.hasOwnProperty(action)&&actions[action]){this.activateButton(action,!1)}}},handleGlobalOptions:function(){if('1'===MywebACOptions.focusable){this.cache.$body.addClass('myweb-ac-focusable')}
if('1'===MywebACOptions.remove_link_target){$('a[target="_blank"]').attr('target','')}
if('1'===MywebACOptions.add_role_links){$('a').attr('role','link')}},init:function(){this.cacheElements();this.buildElements();this.bindEvents();this.handleGlobalOptions()}};$(document).ready(function($){Myweb_Accessibility_App.init();Myweb_Accessibility_App.setFromLocalStorage()});$(document).ready(function(){$("#myweb-overlay-window-close").on("click",function(){$('#myweb-overlay-window').hide()})})}(jQuery,window,document))